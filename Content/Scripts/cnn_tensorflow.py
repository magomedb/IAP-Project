...
class CNN:
  ...
  #      conv1 = self.conv2d(input_obs, convolutional_weights['wc1'], convolutional_biases['bc1'])
         conv1 = tf.layers.conv2d(inputs = input_obs, filters = 32, kernel_size = 5, strides = 1, padding = 'SAME', activation = tf.nn.relu)
  ...
  #      conv2 = self.conv2d(conv1, convolutional_weights['wc2'], convolutional_biases['bc2'])
         conv2 = tf.layers.conv2d(inputs = conv1, filters = 32, kernel_size = 3, strides = 1, padding = 'SAME', activation = tf.nn.relu)
         conv2 = tf.layers.max_pooling2d(conv2, 2, 2)
  #      conv2 = self.maxpool2d(conv2, k=2)

    with tf.name_scope("Layer1") as scope:
      #W1shape = [self.observation_shape, self.hidden_size]
      W1shape = [2048, self.hidden_size]
    ...
    ue.log(str(self.observation_shape))
    ue.log(str(conv2.shape))
